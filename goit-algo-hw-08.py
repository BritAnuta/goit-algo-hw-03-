...
# Створюємо функціонал для серіалізації наших даних
def save_data(book, filename="addressbook.pkl"):
    with open(filename, "wb") as f:
        pickle.dump(dict(book), f)  # обертаэмо наш AddressBook в dict, щоб коректно провести серіалізацію 

# Створюємо функціонал для десеріалізації наших даних
def load_data(filename="addressbook.pkl"):
    try: # Обробляємо помилку ненаявності файлу
        with open(filename, "rb") as f:
            return AddressBook(pickle.load(f)) # повертаємо адресну книгу з введеними і збереженими даними(словником даних)
    except FileNotFoundError:
        return AddressBook()  # повертаємо нову адресну книгу, якщо файл не знайдено

# CLI-бот
def main():
    book = load_data() # викликаємо функцію для завантаження даних(функцію десеріалізації)

    # Перевіряємо наявність серіалізованих даних і виводимо інформацію наявності
    filename = "addressbook.pkl"
    if os.path.exists(filename):
        print("Serialized file exists.")
    else:
        print("Serialized file does not exist.")
    ...
    while True:
        ...
        if command in ["close", "exit"]:
            save_data(book) # забезпечуємо збереження даних у файлі перед закриттям програми, якщо треба
            print("Good bye!")
            break
        elif command == "hello":
